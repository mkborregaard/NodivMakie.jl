...
- Division of labour: analysis (computations such as `clade_richness`, `sos_ordination`,
  `sos_clusters`) goes in Nodiv, plotting here, anything specific to the workshop's data
  in NodivWorkshop. The docs here must not point to the workshop.
- New function names use underscores for multi-word names and are never capitalized
  (Makie recipe names such as `treeplot` follow from their types).
...
- CSV 1.x reads names as `DataString`; EcoBase 0.2's `asindices` then cannot match them
  ...
  when building assemblages. Upstream issue in EcoBase.
